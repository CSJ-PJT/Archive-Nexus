...
ArchiveOS 실시간 관제에서 서비스가 정지된 것처럼 보이지 않도록 Nexus는 local/demo 환경에서 제한된 속도의 synthetic runtime work loop를 실행할 수 있습니다.

- 기본 API: `GET /api/runtime/status`
- Compose 기본값: `ARCHIVE_RUNTIME_AUTORUN_ENABLED=true`
- 기본 tick: `ARCHIVE_RUNTIME_TICK_INTERVAL=30s`
- tick당 최대 이벤트: `ARCHIVE_RUNTIME_MAX_EVENTS_PER_TICK=10`
- tick당 최대 backlog 수요: `ARCHIVE_RUNTIME_MAX_BACKLOG_PER_TICK=50`

루프는 `MARKET_ORDER_PLACED`, `PRODUCTION_REQUESTED`, `SHIPMENT_REQUESTED`를 기존 Market inbound 서비스로 수신시켜 Workforce capacity와 Outbox routing을 실제로 통과시킵니다. Summary API는 read-only이며 데이터를 생성하지 않습니다.
...
| `GET` | `/api/runtime/status` | Autonomous runtime work loop 상태 |
