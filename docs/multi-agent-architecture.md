...
flowchart LR
    ...
    Router --> I[InventoryAgent]
    Router --> L[LogisticsAgent]
    Router --> X[CrossDomainRootCauseAgent]
...
2. Intent Router가 `PRODUCTION`, `QUALITY`, `MAINTENANCE`, `INVENTORY`, `LOGISTICS`, `CROSS_DOMAIN`, `UNKNOWN` 중 하나 이상을 반환한다.
...
- Inventory와 Logistics Agent는 기존 재고·출하 조회를 재사용한다. Cross Domain Agent는 품질 → 생산 → 재고 → 정비 → 출하의 근거를 한 결과로 연결한다.
- Forecast Agent는 구현하지 않는다.
...
`ScenarioService`는 별도 제조 데이터를 만들지 않고 기존 `NexusStateService`에 결정적 이상 상태를 주입한다.

```mermaid
flowchart LR
    Sensor[센서 이상] --> Quality[품질 불량]
    Quality --> Inventory[재고 부족]
    Inventory --> Logistics[출하 지연]
    Logistics --> Analysis[Cross Domain 분석]
    Analysis --> Approval[ArchiveOS PM 승인]
    Approval --> RPA[기존 RPA 실행]
    RPA --> Verify[결과 검증 및 Callback]
```

Production, Inspection, Maintenance, Approval은 현재 snapshot을 대체하지 않는 bounded projection table로 먼저 분리한다. Projection 실패가 원본 simulator state를 변경하지 않으며 이후 독립 aggregate repository로 점진 전환한다.
