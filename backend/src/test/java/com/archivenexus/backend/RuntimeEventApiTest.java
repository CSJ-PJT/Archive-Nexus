...
class RuntimeEventApiTest {
    ...
    void operationsSummaryIncludesLiveFlowContract() throws Exception {
        mvc.perform(get("/api/operations/summary"))
                ...
                .andExpect(jsonPath("$.economy.manufacturingRevenue").value(0))
                .andExpect(jsonPath("$.economy.materialCost").value(0))
                .andExpect(jsonPath("$.economy.operatingMargin").value(0))
                .andExpect(jsonPath("$.economy.cashBalance").value(nullValue()))
                .andExpect(jsonPath("$.economy.negativeProfitStreak").exists())
                .andExpect(jsonPath("$.economy.status").value("NO_ACTIVITY"))
                .andExpect(jsonPath("$.economy.available").value(true))
                .andExpect(jsonPath("$.economy.financialEventCount").value(0))
